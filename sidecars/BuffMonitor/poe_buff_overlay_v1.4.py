"""
Standalone PoE2 Rend overlay prototype v1.4.
...
Run:
  python.exe .\poe_buff_overlay_v1.4.py
...
# ----------------------------- EASY SETTINGS -----------------------------
OVERLAY_X = 1650               # Initial screen X position
OVERLAY_Y = 760                # Initial screen Y position
SCALE = 1.25                   # 1.0 = original size, 1.25 = 25% larger
DETECTION_INTERVAL_MS = 200    # Re-check position 5 times per second
DISPLAY_INTERVAL_MS = 50       # Refresh live copied pixels at 20 FPS
EXTRA_BELOW_ICON = 28          # Pixels below icon to include timer text
PADDING = 0
TRANSPARENT_COLOUR = "#ff00ff"  # exact color key, intentionally uncommon
SHOW_BORDER = False            # True = visible border
BORDER_WIDTH = 1               # Used only when SHOW_BORDER is True
...
def load_configuration():
    if not CONFIG_FILE.exists():
        fail("detector_config.json is missing. Put this script in the same folder as your calibrated detector files.")
    try:
...
def make_inactive_image(template, power_available):
    ...
    canvas = np.full((h + EXTRA_BELOW_ICON, w, 3), (255, 0, 255), dtype=np.uint8)
    canvas[:h, :w] = dim
    cv2.rectangle(canvas, (0, 0), (w - 1, h - 1), (105, 105, 105), max(1, round(min(w, h) * 0.025)))
    ...
    if power_available:
        ...
        cv2.line(canvas, (cx - arm, cy), (cx + arm, cy), (0, 255, 0), thickness, cv2.LINE_AA)
        cv2.line(canvas, (cx, cy - arm), (cx, cy + arm), (0, 255, 0), thickness, cv2.LINE_AA)
    return canvas
...
class BuffOverlay:
    def __init__(self):
        ...
        self.misses_needed = int(self.cfg.get("misses_before_gone", 2))
        ...
        self.frame = tk.Frame(
            self.root, bg=TRANSPARENT_COLOUR,
    ...
    def register_hotkeys(self):
        ...
        self.key_latches = {win32con.VK_F8: False}
    ...
    def toggle_visibility(self):
        ...
        if self.visible:
            ...
            self.root.withdraw()
        print(f"Overlay: {'VISIBLE' if self.visible else 'HIDDEN'}")


    def capture_client_roi(self, hwnd, roi):
        ox, oy = client_origin(hwnd)
        x, y, w, h = map(int, roi)
        raw = np.asarray(self.sct.grab({"left": ox + x, "top": oy + y, "width": w, "height": h}))
    ...
    def detection_tick(self):
        try:
            ...
            self.root.after(DETECTION_INTERVAL_MS, self.detection_tick)

    def display_tick(self):
        try:
            hwnd = find_game_window()
            if (self.state["rend"] and self.rend_box_client and hwnd
                    and not win32gui.IsIconic(hwnd)):
                x, y, w, h = self.rend_box_client
                cw, ch = client_size(hwnd)
                extra = max(0, min(EXTRA_BELOW_ICON, ch - (y + h)))
            ...
            self.root.after(DISPLAY_INTERVAL_MS, self.display_tick)

    def render_inactive(self):
        image = make_inactive_image(self.templates["rend"], self.state["power_charge"])
        self.show_image(image)

    def show_image(self, image):
        self.photo = bgr_to_photo(image, SCALE)
    ...
    def run(self):
        ...
        print("F8 = show/hide | Esc = exit | Overlay is permanently click-through")
        print("v1.4: real top-level HWND, color-key transparency, permanent click-through.")
        self.root.mainloop()
